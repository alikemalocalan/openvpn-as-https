...
if grep -qs "ubuntu" /etc/os-release; then
  ...
  apt-get install -y iproute2 wget curl tzdata dnsutils systemd apt-utils
  # set your timezone
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
  ...
  # If system has a single IPv4, it is selected automatically. Else, ask the user  if [[ $(ip -4 addr | grep inet | grep -vEc '127(\.[0-9]{1,3}){3}') -eq 1 ]]; then
